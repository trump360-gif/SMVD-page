 ...
 *
 * 실행 방법:
 *   node --loader ts-node/esm scripts/setup-studio-knot-cms.ts
 */
...
async function main() {
  ...
  try {
    ...
    studioKnotBlogContent.blocks.forEach((block, idx) => {
      console.log(`   - Block ${idx}: ${(block as any).type}`);
    });
    ...
    console.log(`   - Blocks: ${(updated.content as any).blocks.length}`);
    console.log(`   - Rows: ${(updated.content as any).rowConfig.length}`);
    ...
    console.log('   ✅ 4 blocks configured');
    console.log('   ✅ 3 rows layout applied');
    console.log('   ✅ 9 gallery images defined');
    console.log('   ✅ Data saved to database\n');

  } catch (error) {
    console.error('❌ Error during Phase 2:', (error as Error).message);
    process.exit(1);
  } finally {
